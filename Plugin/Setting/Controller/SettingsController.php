 ...
 */
	...
	public function admin_import($id = null) {
		...
		if ($this->request->is('post')) {
			...
			$objresult = $this->Common->upload_and_read_excel($data['Setting'], '');
			...
			foreach($data_upload as $obj) {
				...
				if ($line > 0) {
					...
					if ($valid) {
						...
						$data_insert[$model]['slug'] = $obj[1];
						$data_insert[$model]['value'] = $obj[2];
						$data_insert[$model]['description'] = $obj[3];
