    ...
    <div class="col-xs-12">
		<div class="box box-primary">
			<div class="box-header">
				<h3 class="box-title"><?= __d('schedule', 'past_item_title'); ?></h3>
                ...
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane active" id="info-tab">
                        ...
                        foreach($data_detail as $data_detail) {
                            $info_order = isset($past_schedule_info[$data_detail['ScheduleDetail']['id']]) ? $past_schedule_info[$data_detail['ScheduleDetail']['id']] : array();
                        ?>
                        <div class="div-container">
                            <div class="div-detail">
                                <div class="div-time"><strong><?= __('time') ?></strong> : <?= date('H:i', strtotime($data_detail['ScheduleDetail']['time'])) ?></div>
                                <div class="div-time"><strong><?= __('Total Price : ') ?></strong> : <?php echo isset($info_order['total']) ? $info_order['total'] . ".00" : 0 ?></div>
<!--                                <div class="div-price">
                                    <strong><?/*= __('total_ticket_by_price') */?></strong>
                                    <ul>
                                        <li><?/*= 'Total Adult' */?> : <?/*= isset($info_order['total_adult']) ? $info_order['total_adult'] : 0 */?></li>
                                        <li><?/*= 'Total Student ' */?> : <?/*= isset($info_order['total_student']) ? $info_order['total_student'] : 0 */?></li>
                                        <li><?/*= 'Total Senior ' */?> : <?/*= isset($info_order['total_senior']) ? $info_order['total_senior'] : 0 */?></li>
                                        <li><?/*= 'Total Child ' */?> : <?/*= isset($info_order['total_child']) ? $info_order['total_child'] : 0 */?></li>
                                    </ul>
                                </div>-->
                                <div class="div-price">
                                    <strong><?= __('total_ticket_is_sold') ?></strong>
                                    <ul>
                                        <?php
                                        foreach($data_detail['ScheduleDetailTicketType'] as $price) {
                                            ?>
                                            <li><?= $price['TicketType']['code'] ?> : <?= isset($info_order[$price['TicketType']['code']]) ? $info_order[$price['TicketType']['code']] : 0 ?></li>
                                            <?php
                                        }
                                        ?>
                                    </ul>
