...
<?= $this->element('Movie.star_filter', array(
		...
		<div class="box box-primary">
			<div class="box-header">
                ...
                <?php if(isset($permissions[$model]['add']) && ($permissions[$model]['add'] == true)){ ?>
                    <?= $this->Html->link( '<i class="glyphicon glyphicon-plus"></i> ' . __d('movie', 'add_star'), array('action' => 'add'), array('class' => 'btn btn-primary', 'escape' => false)); ?>
                <?php } ?>
				</div>
			</div>	

			<div class="box-body table-responsive">
				<table id="Stars" class="table table-bordered table-striped">
					<thead>
						<tr>
							<th class="text-center"><?= $this->Paginator->sort('id', __('id')); ?></th>
							<th class="text-center"><?= $this->Paginator->sort('first_name', __d('movie', 'code_first_name')); ?></th>
							<th class="text-center"><?= $this->Paginator->sort('surname', __d('movie', 'code_surname')); ?></th>
                            <th class="text-center"><?= $this->Paginator->sort($languages_model.'.first_name', __d('movie', 'first_name')); ?></th>
                            <th class="text-center"><?= $this->Paginator->sort($languages_model.'.surname', __d('movie', 'surname')); ?></th>
					...
					<tbody>
                        <?php foreach ($dbdatas as $dbdata): ?>
							<tr>
                                ...
                                <td class="text-center">
									<?= h($dbdata[$languages_model]['star_first_name']); ?>
								</td>			        

								<td class="text-center">
									<?= h($dbdata[$languages_model]['star_surname']); ?>
								</td>
