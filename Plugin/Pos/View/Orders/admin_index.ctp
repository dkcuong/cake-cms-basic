...
<?= $this->element('Pos.order_filter', array(
				...
				<table id="Orders" class="table table-bordered table-striped">
					<thead>
						<tr>
                            ...
                            <th class="text-center"><?= __d('order','amount_of_ticket'); ?></th>
							...
							<tr>
                                ...
                                <td class="text-center">
                                    ...
                                    if (!empty($dbdata['MovieLanguage']['name'])) {
                                        ...
                                        echo $this->Html->link($dbdata['MovieLanguage']['name'] . ' (' . $dbdata['MovieType']['name'] . ')', array(
                                            'plugin' => 'movie', 'controller' => 'movies',
                                            'action' => 'view', 'admin' => true, 'prefix' => 'admin', $dbdata['MovieLanguage']['movie_id']
                                        ));
                                    }
                                    ?>
                                </td>
                                <td class="text-center">
                                    <?php
                                    $date_schedule = $dbdata['ScheduleDetail']['date'];
                                    $date_schedule = date('Y-m-d', strtotime($date_schedule));
                                    $time_schedule = $dbdata['ScheduleDetail']['time'];
                                    $time_schedule = date('H:i', strtotime($time_schedule));

                                    echo $date_schedule . " " . $time_schedule;
                                    ?>
                                </td>
                                <td class="text-center">
                                    <?php echo $dbdata[0]['amount_of_ticket']; ?>
                                </td>
                                <!--<td class="text-center">
                                ...
                                <td class="text-center">
                                    ...
                                    /*= $status[$dbdata[$model]['qr']]; */
                                    $payment_method = '';
                                    if (isset($dbdata[$model]['is_pos']) && $dbdata[$model]['is_pos'] == 1) {
                                        $payment_method = $dbdata[0]['payment_method_group'];
                                    } else {
                                        $payment_method = !empty($dbdata["OrderPaymentLog"]["payType"]) ?  strtoupper($dbdata["OrderPaymentLog"]["payType"]) : ' ';
                                    }
                                    echo $payment_method;
                                    ?>
                                    <?php //echo strtoupper($dbdata['OrderPaymentLog']['payType']); ?>
                                </td>
                                <td class="text-center">
