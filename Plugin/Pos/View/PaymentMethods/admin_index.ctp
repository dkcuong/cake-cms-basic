...
<?= $this->element('Pos.payment_method_filter', array(
		...
		<div class="box box-primary">
			<div class="box-header">
                ...
                <?php if(isset($permissions[$model]['add']) && ($permissions[$model]['add'] == true)){ ?>
                    <?= $this->Html->link( '<i class="glyphicon glyphicon-plus"></i> ' . __d('payment', 'add_method'), array('action' => 'add'), array('class' => 'btn btn-primary', 'escape' => false)); ?>
                <?php } ?>
				</div>
			</div>	

			<div class="box-body table-responsive">
				<table id="PaymentMethods" class="table table-bordered table-striped">
					<thead>
						<tr>
							...
							<th class="text-center"><?= $this->Paginator->sort('value', __d('payment', 'method_value')); ?></th>
