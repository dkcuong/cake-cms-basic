...
class ItemGroupsController extends PosAppController {

	public $components = array('Paginator');
	private $model = 'ItemGroup';
	private $model_lang = 'ItemGroupLanguage';
	...
	public function beforeFilter(){	
		parent::beforeFilter();
		$this->set('title_for_layout', __d('item_group', 'item_title'));
	}
		...
		$this->Paginator->settings = array(
			...
			'joins' => array(
				array(
					...
					'table' => Environment::read('table_prefix') . 'item_group_languages',
					'type' => 'left',
					'conditions' => array(
						$model.'.id = '.$languages_model.'.item_group_id',
	...
	public function admin_export(){
	   ...
	   if( $this->request->is('get') ) {
		   ...
		   if ($result) {
			   ...
			   try{
				   $file_name = 'item_groups_'.date('Ymd');
	...
	public function admin_import($id = null) {
		...
		if ($this->request->is('post')) {
			...
			$objresult = $this->Common->upload_and_read_excel($data['ItemGroup'], '');
				...
				if ($line > 0) {
					...
					if ($valid) {
						...
						if (isset($data_old[$model]['id']) && !empty($data_old[$model]['id'])) {
							...
							$data_lang = $this->$model->$languages_model->find('all', array('conditions' => array('item_group_id' => $data_old[$model]['id'])));
