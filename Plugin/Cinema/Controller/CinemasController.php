...
class CinemasController extends CinemaAppController {

	public $components = array('Paginator');
	private $model = 'Cinema';
	//private $model_lang = 'CinemaLanguage';
	...
	public function beforeFilter(){	
		parent::beforeFilter();
		$this->set('title_for_layout', __d('place', 'cinema_title'));
	}
			   ...
			   try{
				   ...
				   if ($this->request->type == "xls") {
						$excel_readable_header = array(
							...
							array('label' => __d('place', 'location')),
	...
	public function admin_import($id = null) {
		...
		if ($this->request->is('post')) {
			...
			$objresult = $this->Common->upload_and_read_excel($data['Cinema'], '');
			...
			foreach($data_upload as $obj) {
				...
				if ($line > 0) {
					...
					if ($valid) {
						...
						$data_insert[$model]['address'] = $obj[2];
						$data_insert[$model]['location'] = $obj[3];
						$data_insert[$model]['description'] = $obj[4];
						$data_insert[$model]['phone'] = $obj[5];
						$data_insert[$model]['email'] = $obj[6];
						$data_insert[$model]['enabled'] = (in_array($obj[7], array('Y', 'y'))) ? 1: 0;
